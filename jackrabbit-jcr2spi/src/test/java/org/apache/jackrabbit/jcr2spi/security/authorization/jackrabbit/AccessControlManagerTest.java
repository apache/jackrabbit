...
public class AccessControlManagerTest extends AbstractAccessControlTest {
    ...
    @Test
    public void testRemoveTransientlyAddedPolicy() throws Exception {
        try {
            AccessControlPolicyIterator applicable = getACManager()
                    .getApplicablePolicies(path);
            AccessControlPolicy acl = applicable.nextAccessControlPolicy();
            getACManager().setPolicy(path, acl);

            JackrabbitAccessControlList policy = (JackrabbitAccessControlList) getACManager()
                    .getPolicies(path)[0];
            assertEquals(0, policy.size());
            getACManager().removePolicy(path, policy);
        } finally {
    ...
    public void testGetPolicesAfterASetPoliciesCall() throws Exception {
        AccessControlPolicyIterator policies = getACManager()
                .getApplicablePolicies(path);
    ...
    /**
     * This should be able to return the policies that has been transiently
     * added to the node at path, as the getPolicies api specifies that the
     * method should take the transient changes into account.
     * 
     * @throws Exception
     */
    @Test
    public void testRemovePolicyAfterASetPoliciesCall() throws Exception {
        try {
            ...
            AccessControlPolicy[] unsavePolicy = getACManager().getPolicies(
                    path);

            // MUST be able to get policies that are not ineffect for the node
            // at 'path'
    ...
    public void testGetPoliciesAfterASaveCall() throws Exception {
        ...
        try {
            ...
            AccessControlPolicyIterator pi = getACManager()
                    .getApplicablePolicies(path);
            assertTrue(pi.hasNext());
            AccessControlListImpl policy = (AccessControlListImpl) pi
            ...
            policy.addAccessControlEntry(getUnknownPrincipal(),
                    getPrivileges(0));
            ...
            assertEquals(getPrivileges(1)[0].getName(),
                    policy.getAccessControlEntries()[0].getPrivileges()[0]
                            .getName());

            // ... and he is revoked the 'read' privilege
            assertEquals(getPrivileges(0)[0].getName(),
                    policy.getAccessControlEntries()[1].getPrivileges()[0]
                            .getName());
